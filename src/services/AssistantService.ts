...
const VITE_PRODUCTION_API_BASE_URL = import.meta.env.VITE_PRODUCTION_API_BASE_URL as string;
...
export const sendPromptToAssistant = async (
  prompt: string,
  threadId?: string
): Promise<any> => {
  try {
    // Construct the endpoint URL using the base URL from the environment variable.
    const endpoint = `${VITE_PRODUCTION_API_BASE_URL}/assistant`;
